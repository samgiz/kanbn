 ...
 */
export function validateOptions (options: IndexOptions) {
 ...
 */
function validateColumns (columns: any) {
  ...
  if (result.errors.length) {
    throw new Error(result.errors.map((error: any) => `${error.property} ${error.message}`).join('\n'))
  }
}
 ...
 * @return {object}
 */
export function md2json (data: string): any {
  let name: string = ''; let description: string = ''; let options: any = {}; let columns: any = {}
  try {
    // Check data type
    if (!data) {
    ...
    // Get description
    description = index[name]?.content.trim() ?? ''
        ...
        try {
          return [
            ...
            index[columnName].content
              ? marked.lexer(index[columnName].content)[0].items.map((item: any) => item.tokens[0].tokens[0].text)
              : []
          ]
        } catch (error) {
  ...
  return new Index(name, description, options, columns)
}
 ...
 * @return {string}
 */
export function json2md (data: any, ignoreOptions = false) {
  const result = []
  try {
    // Check data type
    if (!data) {
    ...
    for (const column in data.columns) {
      result.push(
        `## ${column}`,
        data.columns[column].map((taskId: string) => `- [${taskId}](tasks/${taskId}.md)`).join('\n')
      )
    }
  } catch (error: any) {
